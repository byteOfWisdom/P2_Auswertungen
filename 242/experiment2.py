...
def t_to_v(droplet_data, s, dt):
	...
	v_drop = [droplet_data[0], s / t_0, s / t_up, s / t_down]
	return v_drop
...
def find_elemental_charge(charges, preview):
	...
	charges, _ = tools.p.unzip(charges)
	...
	if not preview:
		write_printable({
			"alpha'": [round(i, 3) for i in best_match],
			'N': (best_match * best_n),
			'e_si': ev(charges / (best_match * best_n), 0.0),
			}, 'results/242h.csv', 3)

	return tools.np.sum(charges / (tools.np.round(best_match) * best_n)) / len(charges), charges / (best_match * best_n)

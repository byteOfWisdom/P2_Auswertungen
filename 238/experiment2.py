...
def c(preview, data=None):
    ...
    U_a = ev([round(v, 1) for v in data['data']['data'][U_a_index]], 0.1)
    U_b = ev([round(v, 1) for v in data['data']['data'][U_b_index]], 0.1)
    I_a = ev([round(v, 2) for v in data['data']['data'][I_a_index]], 0.01)
    I_b = ev([round(v, 2) for v in data['data']['data'][I_b_index]], 0.01)
    P_a = ev([round(v, 1) for v in data['data']['data'][P_b_index]], 0.5)
    P_b = ev([round(v, 1) for v in data['data']['data'][P_a_index]], 0.5)
